...
std::vector<double> x = { -5.0,    -3.0,    -1.0,    1.0,    3.0,    5.0 };
std::vector<double> y = { -1.3734,    -1.249,    -0.7854,    0.7854,    1.249,    1.3734 };
...
double errorSquare(std::vector<double> a, std::vector<double> x, std::vector<double> y) {
    double sum = 0;
    for (int i = 0; i < x.size(); ++i) {
        sum += pow(P(a, x[i]) - y[i], 2);
    }
    return sum;
}
...
int main() {
    ...
    std::cout << "]\n";
    std::cout << "the sum of the error square: " << errorSquare(a, x, y) << "\n";
    std::cout << "==================================\n";

    std::cout << "Least squares method, 2st degree\n";
    a = MNK(x, y, 2);
    std::cout << "a coefficients: [";
    for (int i = 0; i < a.size(); ++i) std::cout << " " << a[i] << " ";
    std::cout << "]\n";

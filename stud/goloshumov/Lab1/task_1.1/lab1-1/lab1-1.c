    ...
    for (int j = 0; j < size; j++) {
        ...
        for (int i = j + 1; i < size; i++)
            if (absolute(LUP[1]->data[i][j]) > absolute(LUP[1]->data[max][j]))
                max = i;
        matrix_exchange_strings(LUP[1], j, max);
        matrix_exchange_strings(LUP[2], j, max);
        matrix_exchange_strings(LUP[0], j, max);
        matrix_exchange_columns(LUP[0], j, max);
        for (int i = j + 1; i < size; i++) {
...
Matrix* LU_solve(Matrix** LUP, Matrix* vector) {
    ...
    if (!LUP)
        return NULL;
    result = multiplication(LUP[2], vector);
...
Matrix* matrix_inversion(Matrix* matrix) {
    ...
    for (j = 0; j < inverse->width; j++) {
        ...
        if (j != inverse->width - 1)
            matrix_exchange_strings(right_part, j + 1, j);
        remove_matrix(temp);
        free(temp);
    }
    {
...
int main(void) {
    ...
    if (result = gauss_method(matrix, vector)) {
        printf("Solve of equation\n");

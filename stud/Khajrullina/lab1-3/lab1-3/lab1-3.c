...
int estimation(double eps, double a, double b) {
...
Matrix* seidel_method(Matrix* alpha, Matrix* betta, double epsilon) {
    ...
    if (alpha_norm != 1) {
        printf("Estimate of number of iteration: k + 1 less then %d\n",
            estimation(epsilon, alpha_norm, matrix_norm(betta)));
    }
...
Matrix* simple_iteration_method(Matrix* matrix, Matrix* vector, double epsilon) {
    ...
    result = seidel_method(alpha, betta, epsilon);
    remove_matrix(alpha);
    remove_matrix(betta);
    free(alpha);
    free(betta);
...
int main(void) {
    ...
    FILE* fmatrix, * fvector, * fepsilon;

    fmatrix = fopen("lab1-3m.txt", "r");
    fvector = fopen("lab1-3v.txt", "r");
    fepsilon = fopen("lab1-3e.txt", "r");
